<template>
    ...
    <button @click="mergeUp()">merge up</button>
    <button @click="mergeDown()">merge down</button>
    <button @click="mergeLeft()">merge left</button>
    <button @click="mergeRight()">merge right</button>
</template>
...
export default {
    ...
    methods: {
        ...
        mergeUp() {
            ...
            for (let i = 0; i < this.size; i++) {
                ...
                for (let j = 0; j < this.size - 1; j++) {
                    // 须事先判断是否为空位, 否则 null * null = 0, null + null =0
                    if (this.array[j][i] === this.array[j + 1][i] && this.array[j][i] !== null && this.array[j + 1][i] !== null) {
                        // 行数小者倍增 (j < (j + 1))
                        this.array[j][i] *= 2;
                        // 行数大者置空 ((j + 1) > j)
        ...
        mergeDown() {
            ...
            for (let i = this.size - 1; i > 0; i--) {
                for (let j = 0; j < this.size; j++) {
                    if (this.array[i][j] === this.array[i - 1][j] && this.array[i][j] !== null && this.array[i - 1][j] !== null) {
                        // 行数大者倍增
                        this.array[i][j] *= 2;
                        // 行数小者置空
        ...
        // 1.3  mergeRight() 右 (向右合并)
        mergeRight() {
            for (let i = 0; i < this.size; i++) {
                // 内层 j 倒置循环
                // 内层须少循环一次, 避免 j - 1 超出数组索引范围 (j 不含 0)
                for (let j = this.size - 1; j > 0; j--) {
                    if (this.array[i][j] === this.array[i][j - 1] && this.array[i][j] !== null && this.array[i][j - 1] !== null) {
                        // 列数大者倍增
                        this.array[i][j] *= 2;
                        // 列数小者置空
        ...
        // 在数组空位 (null) 上生成新的随机数,个数为 this.newNumbers
        // 二维数组先拍平, 再过滤掉假值(这里即 null)后取第一个元素
        addRandomNumber() {
