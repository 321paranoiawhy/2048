        ...
        <div class="layout">
            ...
            >
                <div
                    ...
                    class="column"
                    :class="{
                        class1: row[columnIndex] === base,
                        class2: row[columnIndex] === base ** 2,
                        class3: row[columnIndex] === base ** 3,
                        class4: row[columnIndex] === base ** 4,
                        class5: row[columnIndex] === base ** 5,
                        class6: row[columnIndex] === base ** 6,
                        class7: row[columnIndex] === base ** 7,
                        class8: row[columnIndex] === base ** 8,
                        class9: row[columnIndex] === base ** 9,
                        class10: row[columnIndex] === base ** 10,
                        class11: row[columnIndex] === base ** 11,
                        class12: row[columnIndex] === base ** 12,
                        class13: row[columnIndex] === base ** 13,
                        class14: row[columnIndex] === base ** 14,
                        class15: row[columnIndex] === base ** 15,
                        class16: row[columnIndex] === base ** 16,
                        class17: row[columnIndex] === base ** 17,
                        class18: row[columnIndex] === base ** 18,
                        class19: row[columnIndex] === base ** 19,
                        class20: row[columnIndex] === base ** 20,
                    }"
                >
                    {{ row[columnIndex] }}
                </div>
...
export default {
    ...
    data() {
        return {
            ...
            // 幂 (限定最大 20)
            exponent: 11,
            // 获胜数字 base ** exponent (如 2 ** 11 = 2048)
            winNumber: 2048,
            // undo
            lastArray: [],
            // redo
    ...
    methods: {
        ...
        calculateScores() {
            ...
            this.scores = 0;
            // 开始计分数字
            let beginScoreNumber = this.base * this.base;
            for (
                let i = beginScoreNumber;
                i <= this.winNumber;
                i = i * this.base
            ) {
            ...
            // [2, 2, 3, 4, 5, 5, 6, 6, 6, 6].filter((item) => item === 6).length; // 2
        },
        // 数组转置
        ...
        mergeUp() {
            ...
            // 检查是否含有获胜数字
            if (this.array.flat().indexOf(this.winNumber) !== -1) {
            ...
            if (
                this.canNotMergeUp() &&
                this.array.flat().indexOf(null) === -1
            ) {
            ...
            if (this.array.flat().indexOf(null) === -1) {
                ...
                this.array.every((row) => {
                    row
                        ...
                        .every(
                            (rowItem, rowIndex) => rowItem !== row[rowIndex + 1]
                        ) === true;
                });
                array
                    .slice(0, array.length - 1)
                    .every((item, index) => item !== array[index + 1]);
            }
        },
        // checkVertical() 检查纵向是否可以合并

        // 不能向上合并(从上至下从第一列至最后一列逐次判断) [i][j] [i + 1][j]
        // true 不能向上合并 (false 可以继续向上合并)
        canNotMergeUp() {
            // 如果数组中无空值 且 所有元素都不能向上合并 (各列均不能向上合并)
            if (this.array.flat().indexOf(null) === -1) {
                for (let i = 0; i < this.size - 1; i++) {
                    for (let j = 0; j < this.size; j++) {
                        // 数组中已无空值 且存在可以向上合并的同一列相邻元素
        ...
        // 不能向下合并(从下至上从第一列至最后一列逐次判断) [i][j] [i - 1][j]
        // true 不能向下合并 (false 可以继续向下合并)
        canNotMergeDown() {
            // 如果数组中无空值 且 所有元素都不能向下合并 (各列均不能向下合并)
            if (this.array.flat().indexOf(null) === -1) {
                for (let i = this.size - 1; i > 0; i--) {
                    for (let j = 0; j < this.size; j++) {
                        // 数组中已无空值 且存在可以向下合并的同一列相邻元素
                        if (this.array[i][j] === this.array[i - 1][j])
                            return false;
                    }
                }
                // 数组中无空值且任意同列相邻元素均不能向下合并
                return true;
            }
            // 数组中有空值
            if (this.array.flat().indexOf(null) !== -1) {
        ...
        // 不能向左合并(从左至右从第一行至最后一行逐次判断) [i][j] [i][j + 1]
        // true 不能向左合并 (false 可以继续向左合并)
        canNotMergeLeft() {
            // 如果数组中无空值 且 所有元素都不能向左合并 (各行均不能向左合并)
            if (this.array.flat().indexOf(null) === -1) {
                for (let i = 0; i < this.size; i++) {
                    for (let j = 0; j < this.size - 1; j++) {
                        // 数组中已无空值 且存在可以向左合并的同一列相邻元素
                        if (this.array[i][j] === this.array[i][j + 1])
                            return false;
                    }
                }
                // 数组中无空值且任意同行相邻元素均不能向左合并
                return true;
            }
            // 数组中有空值
            if (this.array.flat().indexOf(null) !== -1) {
        ...
        // 不能向右合并(从右至左从第一行至最后一行逐次判断) [i][j] [i][j - 1]
        // true 不能向右合并 (false 可以继续向右合并)
        canNotMergeRight() {
            // 如果数组中无空值 且 所有元素都不能向右合并 (各行均不能向右合并)
            if (this.array.flat().indexOf(null) === -1) {
                for (let i = 0; i < this.size; i++) {
                    for (let j = this.size - 1; j > 0; j--) {
                        // 数组中已无空值 且存在可以向右合并的同一列相邻元素
                        if (this.array[i][j] === this.array[i][j - 1])
                            return false;
                    }
                }
                // 数组中无空值且任意同行相邻元素均不能向右合并
                return true;
            }
            // 数组中有空值
            if (this.array.flat().indexOf(null) !== -1) {
