...
export default {
    ...
    methods: {
        ...
        transpose() {
            // 将数组转置
            this.array = this.array.map((row) => row.map((item) => item));
            this.array = arr.map((row) => row.map((item) => item));
            var newArray = this.array[0].map(function (col, i) {
        ...
        moveUp() {
            ...
            //         this.size - this.array[i].filter(Boolean).length
            //     ).fill(null);
        ...
        moveRight() {
            for (let i = 0; i < this.size; i++) {
                ...
                // 反向排列即可(右向左)
                this.array[i] = tempArrayOne.concat(tempArrayTwo).reverse();
            }
            // for (let i = 0; i < this.size; i++) {
            //     // 内层须少循环一次, 避免 j - 1 超出数组索引范围 (j 不含 this.size - 1)
