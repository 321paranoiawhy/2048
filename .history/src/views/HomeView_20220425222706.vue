...
export default {
    ...
    methods: {
        ...
        addRandomNumber() {
            let randomNumber = this.initializedArray()
                .flat()
                .filter(Boolean)
            ...
            if (Math.floor(Math.random() * (this.size * this.size - 1))) {}
            do {
                temp.push(Math.floor(Math.random() * (this.size * this.size - 1)));
            } while (this.array[temp] === null);
            this.array[temp] = randomNumber;
            console.log(this.array);
        },
        // TODO 撤回操作
        // undo() 返回上一步(撤回)
