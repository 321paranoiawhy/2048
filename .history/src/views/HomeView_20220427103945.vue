...
export default {
    ...
    methods: {
        ...
        addRandomNumber() {
            // slice() 取 this.newNumbers 个元素
            let randomNumberArray = this.initializedArray()
                .flat()
                .filter(Boolean)
                .slice(0, this.newNumbers);
            // [Math.floor(tempRandomNumber / this.size)][
            //     tempRandomNumber % this.size
            // ];

            // 1. 非空值索引数组: 先过滤掉假值 null, 再映射为 this.array 中的索引
            ...
            for (let i = 0; i < this.newNumbers; i++) {
                truthyIndexArray[positionArray[i]];
                this.array[
