...
export default {
    ...
    methods: {
        ...
        calculateScores() {
            let tempArray = this.array.flat();
            let repeatNumbers = new Map(
                [...new Set(this.array.flat())].map((x) => [
                    x,
                    this.array.flat().filter((y) => y === x).length,
                ])
            );
            // this.initialFillableNumbers[this.initialFillableNumbers.length - 1] 最后一个元素
            // this.base * this.initialFillableNumbers[this.initialFillableNumbers.length - 1] 从此数开始计分 (含此数)
            this.scores =
                this.base *
                this.initialFillableNumbers[
                    this.initialFillableNumbers.length - 1
                ];
        },
        // 数组转置
